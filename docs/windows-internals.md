...
make-installer.sh runs that, builds the shell extension, and compiles the Inno Setup script. The installer runs per user without administrator rights and installs under `%LOCALAPPDATA%\Programs\kitty`. If selected, the PATH entry is written to that user's environment, and the installer migrates away the older entry that pointed at the launcher directory.

The Explorer entry is built twice, because Windows 11 and Windows 10 read different things. Windows 10 gets classic registry verbs in the current user's Classes hive for Directory, Directory\Background, and Drive. Windows 11 ignores those in its main menu and wants a package, so the entry there comes from a sparse MSIX that carries an IExplorerCommand implementation written in plain C. make-shellext.sh compiles that DLL, stages the package, packs it with makeappx, and signs it with a self-signed certificate it creates once. The installer trusts that certificate in the current user's TrustedPeople store and registers the package for that user with Add-AppxPackage and an external location. Registration is optional; if Windows rejects it, kitty remains installed and the installer reports that the Explorer menu could not be enabled.
